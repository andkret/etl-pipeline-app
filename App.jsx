...
function SideNode({ id, data }) {
  return (
    <div className="side-node">
      <button className="node-delete-btn" onClick={() => data.onDelete(id)}>
        🗑️
      </button>
      <Handle
        type="target"
...
function PipelineBuilder() {
  ...
  // --- Callbacks defined up front so template loader can use them ---
  ...
  // 1) On mount, load the starter diagram
  useEffect(() => {
    fetch('architecture-template.json')
      .then((res) => res.json())
      .then(({ nodes: initNodes = [], edges: initEdges = [] }) => {
        let maxIndex = -1;
        const remapped = initNodes.map((n) => {
